        ...
        try {
            ...
            if ($isBuyNow) {
                $checkoutCart = array_values(array_filter($fullCart, function ($item) use ($checkoutId) {
                    ...
                    if (!empty($checkoutId)) return ($item['checkout_id'] ?? null) === $checkoutId;
                    return true;
                }));
            } else {
            ...
            foreach ($mergedCart as $item) {
                ...
                if ($product->product_type === 'variant') {
                    ...
                    if (!$variantId) throw new \Exception("Variant ID missing for: {$product->name}");
                ...
                $orderDetails[] = [
                    ...
                    'matrix'           => !empty($item['matrix']) ? json_encode($item['matrix']) : null,
                    'gst_type'         => 0,
            ...
            if (!empty($cachedCouponId)) {
                ...
                if ($coupon) $coupon_amount = self::getCouponDetails($coupon, $netTotal);
            }
        ...
        if ($status === 'SUCCESS') {
            ...
            if ($paymentRow->is_buy_now) {
                ...
                $remaining = array_values(array_filter($cartItems, function ($item) use ($paymentRow) {
                    if (empty($item['is_buy_now'])) return true;
